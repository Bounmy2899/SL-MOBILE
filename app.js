// =========================================================================
// ໂພນມະນີ — app.js (ເວີຊັນ Firebase)
// -------------------------------------------------------------------------
// ໄຟລ໌ນີ້ຄືສະໝອງຂອງເວັບ. ມັນຖືກແບ່ງເປັນ 5 ສ່ວນ:
//   1) ຕັ້ງຄ່າ Firebase (App / Firestore / Storage / Auth)
//   2) "ແຄສທ້ອງຖິ່ນ" (data) — ສຳເນົາຂໍ້ມູນຫຼ້າສຸດຈາກຖານຂໍ້ມູນທີ່ເກັບໄວ້ໃນໜ່ວຍຄວາມຈຳ
//      ເພື່ອໃຫ້ໂຄ້ດສ່ວນສະແດງຜົນ (render*) ຂຽນງ່າຍຄືເກົ່າ
//   3) Helper functions ນ້ອຍໆ (money, escapeHtml, ...)
//   4) ຟັງຊັນສະແດງຜົນ (render...) — ແປງ data ເປັນ HTML
//   5) ຟັງຊັນຂຽນຂໍ້ມູນ (add/update/delete) — ຄຸຍກັບ Firestore/Storage/Auth
...
// ---------- 2) Local cache (ອັບເດດອັດຕະໂນມັດໂດຍ onSnapshot ຂ້າງລຸ່ມ) ----------
...
function toDate(value) { if (!value) return new Date(0); return typeof value.toDate === "function" ? value.toDate() : new Date(value); }
function dateLabel(value) { return toDate(value).toLocaleString("lo-LA", { dateStyle: "medium", timeStyle: "short" }); }
function orderTotal(order) { return order.items.reduce((sum, item) => sum + item.price * item.quantity, 0); }

// ອັບໂຫລດຮູບໜຶ່ງໃບຂຶ້ນ Firebase Storage, ໄດ້ຄືນ URL ສຳລັບເອົາໄປໃສ່ໃນ Firestore
...
function validCart() {
  cart = cart.filter(line => data.products.some(product => product.id === line.productId));
  return cart.map(line => ({ ...line, product: data.products.find(product => product.id === line.productId) })).filter(line => line.product);
}
function renderCart() {
...
function addToCart(productId) {
  ...
  const inCart = cart.find(line => line.productId === productId);
...
function changeCart(productId, amount) {
  const line = cart.find(item => item.productId === productId); const product = data.products.find(item => item.id === productId); if (!line || !product) return;
  line.quantity += amount; if (line.quantity <= 0) cart = cart.filter(item => item.productId !== productId); if (!isOnDemand(product) && line.quantity > product.stock) { line.quantity = product.stock; toast("ສິນຄ້າມີເທົ່າຈຳນວນສະຕັອກ"); }
  saveCart(); renderCart();
}
...
function renderDashboard() {
  ...
  const recent = [...data.orders].sort((a,b) => toDate(b.createdAt) - toDate(a.createdAt)).slice(0, 5);
...
function renderFinancials() {
  ...
  const expenses = [...data.expenses].sort((a, b) => toDate(b.createdAt) - toDate(a.createdAt));
...
function renderOrders() {
  ...
  const orders = [...data.orders].sort((a,b) => toDate(b.createdAt) - toDate(a.createdAt)).filter(order => {
...
// ---------- 5) Firestore / Storage / Auth writes ----------

// ອໍເດີໃໝ່ຈາກລູກຄ້າ (ບໍ່ຕ້ອງ login — ອະນຸຍາດ create ໄດ້ໃນ Security Rules)
async function createOrderFromForm(form) {
  ...
  await Promise.all(lines.filter(line => !isOnDemand(line.product)).map(line => updateDoc(doc(db, "products", line.product.id), { stock: increment(-line.quantity) })));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---- ຟັງຂໍ້ມູນຈາກ Firestore ແບບ real-time (ອັບເດດອັດຕະໂນມັດທຸກເຄື່ອງ) ----
  onSnapshot(collection(db, "categories"), snap => { data.categories = snap.docs.map(d => ({ id: d.id, ...d.data() })); renderAll(); }, err => console.error("categories:", err));
  onSnapshot(collection(db, "products"), snap => { data.products = snap.docs.map(d => ({ id: d.id, ...d.data() })); renderAll(); }, err => console.error("products:", err));
  onSnapshot(query(collection(db, "orders"), orderBy("createdAt", "desc")), snap => { data.orders = snap.docs.map(d => ({ id: d.id, ...d.data() })); renderAll(); }, err => console.error("orders:", err));
  onSnapshot(collection(db, "expenses"), snap => { data.expenses = snap.docs.map(d => ({ id: d.id, ...d.data() })); renderFinancials(); }, err => console.error("expenses:", err));
  onSnapshot(doc(db, "settings", "store"), snap => {
    const saved = snap.exists() ? snap.data() : {};
    data.profile = { ...defaultProfile, ...(saved.profile || {}) };
    data.payment = { ...defaultPayment, ...(saved.payment || {}) };
  ...
  onAuthStateChanged(auth, user => {
    managerUser = user;
    if (!authReadyOnce) { authReadyOnce = true; if (user) showManager(); }
  });

  $("#customerSearch").addEventListener("input", renderCustomerShop);
  $("#categoryTabs").addEventListener("click", event => { const button = event.target.closest("button[data-category]"); if (!button) return; activeCategory = button.dataset.category; renderCustomerShop(); });
  ...
  $("#cartItems").addEventListener("click", event => { const remove = event.target.closest("[data-cart-remove]"); const change = event.target.closest("[data-cart-change]"); if (remove) { cart = cart.filter(line => line.productId !== remove.dataset.cartRemove); saveCart(); renderCart(); } if (change) changeCart(change.dataset.productId, Number(change.dataset.cartChange)); });
  ...
  $("#loginForm").addEventListener("submit", event => {
    ...
    signInWithEmailAndPassword(auth, fd.get("email").trim(), fd.get("password"))
      .then(() => { $("#loginError").classList.add("hidden"); event.currentTarget.reset(); closeLayers(); showManager(); })
      .catch(() => { $("#loginError").classList.remove("hidden"); });
  });
  $("#logoutButton").addEventListener("click", () => { signOut(auth).then(() => { showShop(); toast("ອອກຈາກລະບົບແລ້ວ"); }); });
  ...
  $("#ordersList").addEventListener("change", event => { const select = event.target.closest("[data-order-status]"); if (!select) return; updateDoc(doc(db, "orders", select.dataset.orderStatus), { status: select.value }).then(() => toast("ອັບເດດສະຖານະອໍເດີແລ້ວ")).catch(() => toast("ອັບເດດບໍ່ສຳເລັດ")); });
  $("#ordersList").addEventListener("click", event => { const receipt = event.target.closest("[data-view-receipt]"); const accept = event.target.closest("[data-accept-order]"); if (receipt) $(`#receipt-${receipt.dataset.viewReceipt}`).classList.toggle("hidden"); if (accept) { updateDoc(doc(db, "orders", accept.dataset.acceptOrder), { status: "accepted" }).then(() => toast("ຮັບອໍເດີແລ້ວ · ພ້ອມດຳເນີນການສັ່ງສິນຄ້າ")); } });
  ...
  $("#productForm").addEventListener("submit", async event => {
    ...
    const old = editingProductId ? data.products.find(product => product.id === editingProductId) : null;
    ...
    try {
      ...
      const product = { name: fd.get("name").trim(), categoryId: fd.get("categoryId"), saleMode: fd.get("saleMode"), description: fd.get("description").trim(), cost: Number(fd.get("cost")), price, stock: Number(fd.get("stock")), supplierUrl: fd.get("supplierUrl").trim(), image };
      if (old) await updateDoc(doc(db, "products", old.id), product); else await addDoc(collection(db, "products"), product);
  ...
  $("#managerProducts").addEventListener("click", event => {
    ...
    if (edit) openProductForm(data.products.find(product => product.id === edit.dataset.editProduct));
    if (remove) { const product = data.products.find(item => item.id === remove.dataset.deleteProduct); if (product && confirm(`ລຶບ “${product.name}” ແທ້ບໍ?`)) { deleteDoc(doc(db, "products", product.id)).then(() => { cart = cart.filter(item => item.productId !== product.id); saveCart(); renderCart(); }); } }
  });
  $("#categoryForm").addEventListener("submit", async event => {
    ...
    await addDoc(collection(db, "categories"), { name }); event.currentTarget.reset(); toast("ເພີ່ມໝວດແລ້ວ");
  });
  $("#categoriesManagerList").addEventListener("click", event => {
    ...
    const used = data.products.some(product => product.categoryId === button.dataset.deleteCategory);
    if (used) return toast("ລຶບໝວດນີ້ບໍ່ໄດ້: ຍັງມີສິນຄ້າຢູ່");
    deleteDoc(doc(db, "categories", button.dataset.deleteCategory));
  });
  $("#paymentForm").addEventListener("submit", async event => {
    ...
    try {
      ...
      await setDoc(doc(db, "settings", "store"), { payment }, { merge: true });
  ...
  $("#expenseForm").addEventListener("submit", async event => {
    ...
    await addDoc(collection(db, "expenses"), { description: fd.get("description").trim(), amount: Number(fd.get("amount")), createdAt: serverTimestamp() });
    event.currentTarget.reset(); toast("ບັນທຶກຄ່າໃຊ້ຈ່າຍແລ້ວ");
  });
  $("#expenseList").addEventListener("click", event => { const button = event.target.closest("[data-delete-expense]"); if (!button) return; deleteDoc(doc(db, "expenses", button.dataset.deleteExpense)).then(() => toast("ລຶບຄ່າໃຊ້ຈ່າຍແລ້ວ")); });
  $("#shopProfileForm").addEventListener("submit", async event => {
    ...
    try {
      ...
      await setDoc(doc(db, "settings", "store"), { profile }, { merge: true });
      toast("ບັນທຶກຂໍ້ມູນຮ້ານແລ້ວ");
    } catch (err) { console.error(err); toast("ບັນທຶກບໍ່ສຳເລັດ"); } finally { submitButton.disabled = false; }
  });

  // ---- ສຳຮອງ / ນຳເຂົ້າຂໍ້ມູນ ----
  // ສົ່ງອອກ: ດຶງຂໍ້ມູນປະຈຸບັນ (ຈາກແຄສທ້ອງຖິ່ນ) ອອກເປັນໄຟລ໌ JSON
  ...
  // ນຳເຂົ້າ: ຂຽນທັບ document ທີ່ id ຕົງກັນໃນ Firestore (ບໍ່ລຶບ document ເກົ່າທີ່ບໍ່ຢູ່ໃນໄຟລ໌)
  // ໝາຍເຫດ: ນີ້ຈະຂຽນທັບຂໍ້ມູນຈິງໃນຖານຂໍ້ມູນອອນລາຍ ທຸກຄົນທີ່ເປີດຮ້ານຈະເຫັນການປ່ຽນແປງທັນທີ
  $("#importData").addEventListener("change", async event => {
    ...
    try {
      ...
      for (const category of imported.categories) await setDoc(doc(db, "categories", category.id || `${Date.now()}-${Math.random().toString(36).slice(2,6)}`), { name: category.name });
      for (const product of imported.products) { const { id, ...rest } = product; await setDoc(doc(db, "products", id || `${Date.now()}-${Math.random().toString(36).slice(2,6)}`), rest); }
      for (const order of imported.orders) { const { id, ...rest } = order; await setDoc(doc(db, "orders", id), rest); }
      for (const expense of (imported.expenses || [])) { const { id, ...rest } = expense; await setDoc(doc(db, "expenses", id || `${Date.now()}-${Math.random().toString(36).slice(2,6)}`), rest); }
      if (imported.profile || imported.payment) await setDoc(doc(db, "settings", "store"), { profile: imported.profile || {}, payment: imported.payment || {} }, { merge: true });
